...
meeterpeter.register_extension('stdapi')
...
def channel_open_stdapi_fs_file(request, response):
	...
	channel_id = meeterpeter.add_channel(meeterpeterFile(file_h))
...
def channel_open_stdapi_net_tcp_client(request, response):
	...
	channel_id = meeterpeter.add_channel(meeterpeterSocketClient(sock))
	response += tlv_pack(TLV_TYPE_CHANNEL_ID, channel_id)
	return ERROR_SUCCESS, response
...
def channel_open_stdapi_net_tcp_server(request, response):
	...
	channel_id = meeterpeter.add_channel(meeterpeterSocketServer(server_sock))
	response += tlv_pack(TLV_TYPE_CHANNEL_ID, channel_id)
	return ERROR_SUCCESS, response
...
@meeterpeter.register_function_windll
...
def stdapi_sys_process_close(request, response):
	...
	del meeterpeter.processes[proc_h_id]
	return ERROR_SUCCESS, response
...
def stdapi_net_socket_tcp_shutdown(request, response):
	...
	channel = meeterpeter.channels[channel_id]
