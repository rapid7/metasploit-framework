...
#
# @abstract Subclass and override {#each_module_ancestor}, {#loadable?}, {#module_path}, and
#   {#read_module_content}.
class Metasploit::Framework::Module::Path::Loader::Base < Metasploit::Model::Base
  #
  # CONSTANTS
  #
  ...
  def load_module(parent_path, type, module_reference_name, options={})
    ...
    try_eval_module = lambda { |namespace_module|
      ...
      # handle interrupts as pass-throughs unlike other Exceptions so users can bail with Ctrl+C
        ...
        rescue Metasploit::Framework::Module::Ancestor::VersionIncompatibility => version_compatibility_error
      ...
      rescue Metasploit::Framwork::Module::Ancestor::Error::MetasploitModuleIncompatibility => error
        load_error(module_path, error)
  ...
  def module_ancestor_reference_name_from_path(path)
