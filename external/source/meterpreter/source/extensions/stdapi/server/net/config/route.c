...
DWORD request_net_config_get_routes(Remote *remote, Packet *packet)
{
	...
	unsigned char dummy_int_name[1] = {0};
	uint32_t dummy_metric = 0;
	...
	do
	{
		     ...
		     index++)
		{
            ...
            // dummy data for WIN32 hosts, should get the correct one
    		route[3].header.type   = TLV_TYPE_STRING;
			route[3].header.length = 1;
			route[3].buffer        = (PUCHAR)&dummy_int_name;

			route[4].header.type   = TLV_TYPE_ROUTE_METRIC;
			route[4].header.length = sizeof(DWORD);
			route[4].buffer        = (PUCHAR)&dummy_metric;
	...
	dprintf("[%s] result = %d, table_ipv4 = %p, table_ipv6=%p", __FUNCTION__, result, table_ipv4,table_ipv6);
	...
	for(index = 0; index < table_ipv4->entries; index++) {
		...
		route[4].header.type   = TLV_TYPE_ROUTE_METRIC;
		route[4].header.length = sizeof(DWORD);
		route[4].buffer        = (PUCHAR)&table_ipv4->routes[index].metric;
		
		packet_add_tlv_group(response, TLV_TYPE_NETWORK_ROUTE, route, 5);
	}
	dprintf("[%s] sent %d IPv4 routes",  __FUNCTION__,index+1);
	// IPv6 routing table
	for(index = 0; index < table_ipv6->entries; index++) {
		...
		route6[4].buffer        = (PUCHAR)&table_ipv6->routes[index].metric;
		
		packet_add_tlv_group(response, TLV_TYPE_NETWORK_ROUTE6, route6, 5);
	}
	dprintf("[%s] sent %d IPv6 routes", __FUNCTION__, index+1);
