...
external/source/meterpreter/source/bionic/compiled/libcrypto.so: posix-meterp-build-tmp/openssl-0.9.8o/libssl.so
	cp posix-meterp-build-tmp/openssl-0.9.8o/libcrypto.so external/source/meterpreter/source/bionic/compiled/libcrypto.so

external/source/meterpreter/source/bionic/compiled/libssl.so: posix-meterp-build-tmp/openssl-0.9.8o/libssl.so
	cp posix-meterp-build-tmp/openssl-0.9.8o/libssl.so external/source/meterpreter/source/bionic/compiled/libssl.so
...
posix-meterp-build-tmp/openssl-0.9.8o/libssl.so:
	...
	(cd posix-meterp-build-tmp/openssl-0.9.8o &&                                                       \
		...
		./Configure --prefix=/posix-meterp-build-tmp/out threads shared no-hw no-dlfcn no-zlib no-krb5 no-idea 386 linux-msf \
	)
	...
	cp posix-meterp-build-tmp/openssl-0.9.8o/libssl.so* posix-meterp-build-tmp/openssl-0.9.8o/libcrypto.so* external/source/meterpreter/source/openssl/lib/linux/i386/

external/source/meterpreter/source/bionic/compiled/libpcap.so: posix-meterp-build-tmp/libpcap-1.1.1/libpcap.so.1.1.1
	cp posix-meterp-build-tmp/libpcap-1.1.1/libpcap.so.1.1.1 external/source/meterpreter/source/bionic/compiled/libpcap.so
...
posix-meterp-build-tmp/libpcap-1.1.1/libpcap.so.1.1.1:
	...
	(cd posix-meterp-build-tmp/libpcap-1.1.1 && patch --dry-run -p0 < ../../external/source/meterpreter/source/libpcap/pcap_nametoaddr_fix.diff && patch -p0 < ../../external/source/meterpreter/source/libpcap/pcap_nametoaddr_fix.diff)
...
depclean:
	...
	find external/source/meterpreter/source/bionic/ -name '*.so' -print0 | xargs -0 rm -f 2>/dev/null
	rm -f external/source/meterpreter/source/bionic/lib*/*.so
	rm -rf external/source/meterpreter/source/openssl/lib/linux/i386/
