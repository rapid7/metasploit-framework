...
class MetasploitModule < Msf::Exploit::Remote
  ...
  def check
	...
	#print_good ("This host is vulnerable")
  ...
  def exploit
    ...
    # from RHOST and RPORT but can be extracted this way
  	# print_status("To_output_text")
    print_status("Extracting ServerName and Port")
      ...
      begin
        ...
        res = send_request_raw(
          ...
          'uri' => target_uri.path
		    ...
		    print_good("request sent")
        if res
          print_error("Server returned status #{res.code}")
          if res.code == 502 || res.code == 400
            next
          elsif session_created?
            return
          else
            print_error("Unknown Response: #{res.code}")
          end
        end
