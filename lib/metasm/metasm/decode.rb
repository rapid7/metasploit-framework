...
class Expression
	...
	def expr_indirections
		...
		when Indirection
			ret << @rexpr
		when ExpressionType
			ret.concat @rexpr.expr_indirections
		else
			ret
		end
	end
...
class CPU
	...
	def replace_instr_arg_immediate(i, old, new)
		i.args.map! { |a|
			case a
			when Expression
				Expression[a.bind(old => new).reduce]
			else
				a
			end
		}
...
class Disassembler
	...
	def add_section(encoded, base)
		...
		when ::String
			...
			raise "invalid section base #{base.inspect} - expected string or integer"
		end
	...
	def each_xref(addr, type=nil)
		...
		when nil
		when ::Array
			@xrefs[addr].each { |x| yield x if not type or x.type == type }
		else
			yield @xrefs[addr] if not type or @xrefs[addr].type == type
		end
	end
...
puts "   backtrace_indirection for #{ind.target} failed: #{ev}" if debug_backtrace
	...
	def backtrace_found_result(expr, di, type, origin, len, detached)
		...
		if di and type == :r and (len == 1 or len == 2) and s = get_section_at(n)
			...
			case len
			when 1
				str = s[0].read(32).unpack('C*')
			when 2
				str = s[0].read(64).unpack('v*')
			end
...
puts "    backtrace_found: addrs_todo << #{n} from #{Expression[origin] if origin}" if debug_backtrace
	...
	def dump_block_header(block, &b)
		...
		each_xref(block.address) { |x|
			case x.type
			when :x
				xr << Expression[x.origin]
			when :r, :w
				xr << "#{x.type}#{x.len}:#{Expression[x.origin]}"
			end
		}
		if not xr.empty?
