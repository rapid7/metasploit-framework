			...
			module ModulePaths
				...
				def init_module_paths(modules=[])
					...
					if (Msf::Config.module_directory)
						self.modules.add_module_path(Msf::Config.module_directory, modules)
					end

					# Initialize the user module search path
					if (Msf::Config.user_module_directory)
						self.modules.add_module_path(Msf::Config.user_module_directory, modules)
					end
					...
					if self.datastore['MsfModulePaths']
						self.datastore['MsfModulePaths'].split(";").each { |path|
							self.modules.add_module_path(path, modules)
						}
					end
				end
			end
