  ...
  module Serializer
    ...
    class ReadableText
      ...
      def self.dump_module_actions(mod, indent = '', h = nil)
        ...
        mod.actions.each_with_index do |target, idx|
      ...
      def self.dump_traits(mod, indent = ' ')
        ...
        unless mod.side_effects.empty?
          ...
          mod.side_effects.each do |side_effect|
        ...
        unless mod.stability.empty?
          ...
          mod.stability.each do |stability|
        ...
        unless mod.reliability.empty?
          ...
          mod.reliability.each do |reliability|
      ...
      def self.options_table(missing, mod, options, indent)
        ...
        options.sort_by(&:name).each do |opt|
          ...
          if name == 'RPORT' && opt.is_a?(Msf::OptPort)
        ...
        framework.jobs.keys.sort_by(&:to_i).each do |job_id|
          ...
          if verbose
            ...
            persist_list.each do |e|
              ...
              if handler_ctx && handler_ctx.respond_to?(:datastore) && e['mod_options']['Options'] == (handler_ctx.datastore.to_h)
                row[7] = 'true'
              end
            end
