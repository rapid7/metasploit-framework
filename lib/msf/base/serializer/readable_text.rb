...
class ReadableText
  ...
  def self.dump_options(mod, indent = '', missing = false)
    options = mod.options.map { |name, option| option }
    options_grouped_by_conditions = options.group_by do |option|
      option.conditions
    end
    ...
    options_grouped_by_conditions.each do |conditions, options|
      ...
      options.sort_by(&:name).each do |opt|
        ...
        tbl << [ opt.display_name || name, opt.display_value(val), opt.required? ? "yes" : "no", desc ]
      end
  ...
  def self.dump_advanced_options(mod, indent = '')
    ...
    options_grouped_by_conditions.each do |conditions, options|
      ...
      options.sort_by(&:name).each do |opt|
        ...
        tbl << [ opt.display_name || name, opt.display_value(val), opt.required? ? "yes" : "no", opt.desc ]
      end
