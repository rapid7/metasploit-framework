      ...
      module CommandDispatcher
        ...
        class Core
          ...
          def cmd_exit(*args)
            ...
            if (framework.sessions.length > 0 and !forced)
              print_status('You have active sessions open, to exit anyway type "exit -y"')
              return
            elsif (driver.confirm_exit and !forced)
          ...
          #
          def cmd_sleep(*args)
            return if !(args and args.length == 1)
          ...
          def cmd_threads(*args)
            ...
            @@threads_opts.parse(args) do |opt, idx, val|
              case opt
              when '-v'
                verbose = true
              when '-l'
          ...
          def cmd_route(*args)
            ...
            when 'print'
              ...
              # Print Route Tables
              print(tbl_ipv4) if tbl_ipv4.rows.length > 0
              print(tbl_ipv6) if tbl_ipv6.rows.length > 0
