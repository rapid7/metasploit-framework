...
def mTdhZAvYmLyAmMXuIZZ4glCPcDT7qdrif
	print_error "Windows platform detected.  This operating system is no longer supported."
	print_error "Please insert 25 cents to continue..."
	$stdout.flush
	total = 0
	while a = $stdin.gets
		if a.to_i != 0
			total += a.to_i
			print_status "#{total} cents inserted so far"
		else
			print_error "Invalid coinage, this prompt accepts only numbers"
		end

		break if total >= 25
	end
	print_good "Console activated"
end
