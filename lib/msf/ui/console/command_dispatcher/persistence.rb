    ...
    module Console
      module CommandDispatcher
        class Persistence
          ...
          def commands
            super.update({
              'run' => 'Launches the persistence module',
              'rerun' => 'Reloads and launches the persistence module',
              'exploit' => 'This is an alias for the run command',
              'rexploit' => 'This is an alias for the rerun command',
              'reload' => 'Reloads the auxiliary module',
              'to_handler' => 'Creates a handler with the specified payload'
            }).merge(mod ? mod.persistence_commands : {})
          end
          ...
          def cmd_run(*args, opts: {})
            ...
            begin
              ...
              print_error('Persistence interrupted by the console user')
            rescue ::Exception => e
              print_error("Persistence failed: #{e.class} #{e}")
              elog('Persistence Failed', error: e)
            end
          end
          ...
          def cmd_run_tabs(str, words)
            fmt = {
              '-e' => [ framework.persistence.module_refnames ],
              '-f' => [ nil ],
              '-h' => [ nil ],
              '-j' => [ nil ],
              '-J' => [ nil ],
