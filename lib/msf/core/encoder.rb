...
class Encoder < Module
  ...
  #
  # Maximum number of randomized key candidates to try in {#find_key} before
  # giving up. This prevents an unbounded loop when no verifying key exists for
  # the supplied badchar set (for example sparc/longxor_tag against a highly
  # restrictive badchar list), which would otherwise spin forever.
  #
  MAX_KEY_SEARCH_ATTEMPTS = 1024
...
protected
  ...
  def find_key(buf, badchars, state)
    ...
    while (!found)
      # Give up if no verifying key has been found within a bounded number of
      # attempts. Returning nil signals the caller to move on to the next
      # encoder rather than spinning forever.
      return nil if (attempts += 1) > MAX_KEY_SEARCH_ATTEMPTS
