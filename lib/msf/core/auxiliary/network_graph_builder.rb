...
module Msf
  # Methods for building the network-graph node/link data used by the
  # auxiliary/analyze/network_graph report and the network_map plugin,
  # including the output sanitization its HTML embedding relies on.
  module Auxiliary::NetworkGraphBuilder
    ...
    def load_workspace_data(ws)
      ...
      all_cve_names = host_vulns.flat_map do |v|
    ...
    # JSON escapes for characters that are significant to the surrounding
    # HTML (<, >, &) or to JavaScript string literals (the U+2028/U+2029
    # line separators, which are legal unescaped inside JSON strings but
    # not inside pre-ES2019 JS strings).  \uXXXX escapes decode back to the
    # exact same characters, so the page sees identical data.
    JSON_SCRIPT_ESCAPES = {
      '&' => '\u0026',
      '<' => '\u003c',
      '>' => '\u003e',
    ...
    # Serializes graph data for embedding inside a <script> element, e.g.
    # the network_map report template.  Database strings (hostnames, SNMP
    # values, loot/vuln info, credential usernames, ...) are
    # attacker-influenced, and JSON.generate leaves markup-significant
    # characters untouched - a value containing '</script>' would
    # terminate the script element early and inject markup into the
    # rendered page.  Escaping every such character means no database
    # value can break out of its JSON string, no matter its content.
    # A Hash gsub replacement performs no backreference processing, so
    # nothing in the data can influence the substitution itself.
    def inline_json(obj)
      json = JSON.generate(utf8_sanitize(obj)).force_encoding('UTF-8')
      json.gsub(/[<&>\u2028\u2029]/, JSON_SCRIPT_ESCAPES)
    end
  end
end
