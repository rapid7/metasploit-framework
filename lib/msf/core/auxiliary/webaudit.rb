...
module Auxiliary::WebAudit
    ...
    def audit_form_methods( method, params )
        generate_parameters( method, params ).each do |form, param, pname|
            resp = submit_request( method, param )
            next if not resp

            if signature.kind_of? ::Regexp
                m = resp.body.match( signature )
                next if not (m and m.length > 1)

                1.upto(m.length-1) do |mi|
                    next if not m[mi]
                    proof = m[mi].gsub(/[\r\n]/, ' ')
    ...
    def submit_request( method, params )
        ...
        begin
            ...
            case method.upcase
                when 'GET'
                    buff = params.map{|param| Rex::Text.uri_encode(param[0].to_s) + "=" + Rex::Text.uri_encode(param[1].to_s)}.join("&")
    ...
    def generate_parameters( method, params )
        ...
        self.target[:forms].each do |form|
            fcnt += 1
            break if fcnt > 5
            ...
            options = form.params.map{|x| x[0] } + ["unknown#{self.token}"]
            options.delete(nil)
            options.delete("")
            ...
            baseline = []
            options.each do |oname|
                baseline << [oname, defaults[oname].to_s]
            end
            ...
            options.each_index do |idx|
                ...
                if params[options[idx]] and params[options[idx]][:values] and params[options[idx]][:values].keys.length > 0
    ...
    def generate_parameters_path_info( params )
        ...
        options = params[:path_info][:values].keys + ['Unknown']

        form = target[:forms].first

        options.each do |input|
            seeds_for( input ).each do |input|
                sig, input = input
                values << [form, sig, [['PATH', input]], :path]
            end
        end
