...
def run
  ...
  if (self.respond_to?('run_host'))
    loop do
      ...
      while (@stl.length < threads_max)
        ...
        @stl << framework.threads.spawn("ScannerHost(#{self.refname})-#{datastore['RHOST']}", false, datastore.dup) do |thr_datastore|
  ...
  if (self.respond_to?('run_batch'))
    ...
    while(true)
      ...
      # and continue on.
      tla = 0
      @stl.map {|t| tla += t[:batch_size] }
      @stl.first.join(1)
      @stl.delete_if { |t| not t.alive? }
      tlb = 0
      @stl.map {|t| tlb += t[:batch_size] }
