...
#}
      ...
      config.scan(/^auth user (.+) \{.+encrypted-password ([$\w\/\+]+)/mi).each do |result|
...
#}
      config.scan(/^sys file ssl-key (.+) \{.+passphrase ([$\w\/\+=]+).+source-path file:\/\/([\w\/\.]+)/mi).each do |result|
...
# previous hash    <EFt+B7/aTWwPwLoMd8KLYW4JB3K5B6301k4pGsoWnZEb2yUbvEJgNU3FcLHo0S4QvdrwVcKrNtHLzebC7HizHQ==>
      ...
      config.scan(/(master-key|previous) hash\s+<([\w\+\/=]+)>/i). each do |result|
