...
#
###
module Msf::Payload::Windows::Prepend
  ...
  def initialize(info = {})
    ...
    register_advanced_options(
      [
        ...
        Msf::OptPath.new('PrependCode', [ false, "File containing code to prepend to the payload" ]),
        Msf::OptPath.new('PrependThread', [ false, "File containing code to prepend as a thread to the payload (Win32 only)" ])
      ], Msf::Payload::Windows )
    ret
  end

  #
  # returns the contents of a provided file and assembles it if the filename ends in '.s'
  #
  def shellcode_from_file(filename)
    code = binread(filename)
    if filename[-2..1] == '.s'
      test_arch = [ *(self.arch) ]
      if test_arch.include?(ARCH_X86)
        assembled_code = Metasm::Shellcode.assemble(Metasm::Ia32.new, code).encode_string
      elsif test_arch.include?(ARCH_X64)
        assembled_code = Metasm::Shellcode.assemble(Metasm::X64.new, code).encode_string
      end
    else
      assembled_code = code
    end
  ...
  #
  # Prepend user-provided shellcode or assembled user-supplied assembly as a thread
  #
  def apply_prepend_thread(buf)
    return buf if datastore['PrependThread'].empty? || ![ *(self.arch) ].include?(ARCH_X86)

    shellcode = shellcode_from_file(datastore['PrependThread'])
    threaded_code = ::Msf::Util::EXE.win32_rwx_exec_thread(shellcode, 0, 'end')
  end
  ...
  #
  # Prepend user-provided shellcode or assembled user-supplied assembly
  #
  def apply_prepend_code(buf)
    return shellcode_from_file(datastore['PrependCode']) + buf unless datastore['PrependCode'].empty?
