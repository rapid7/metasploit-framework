...
class AESCBC(object):
        ...
        for r in xrange(1, rounds):
            for j in xrange(0, 4):
                ...
                self._Kd[r][j] = (self.U1[(tt >> 24) & 0xFF] ^
                                  ...
                                  self.U3[(tt >>  8) & 0xFF] ^
                                  self.U4[ tt        & 0xFF])
    def encrypt_internal(self, plaintext):
        if len(plaintext) != 16:
    ...
    def pad(self, plaintext):
        c = 16 - (len(plaintext) % 16)
        return plaintext + bytes(chr(c) * c, 'utf-8')
    ...
    def encrypt(self, iv, plaintext):
        ...
        return b''.join([self.encrypt_block(b) for b in chunks(plaintext, 16)])
    def encrypt_block(self, plaintext):
        if len(plaintext) != 16:
            raise ValueError('plaintext block must be 16 bytes')
        plaintext = _string_to_bytes(plaintext)
        precipherblock = [ (p ^ l) for (p, l) in zip(plaintext, self._last_cipherblock) ]
        self._last_cipherblock = self.encrypt_internal(precipherblock)
    ...
    def decrypt(self, iv, ciphertext):
        ...
        return self.unpad(b''.join([self.decrypt_block(b) for b in chunks(ciphertext, 16)]))
    def decrypt_block(self, ciphertext):
        if len(ciphertext) != 16:
            raise ValueError('ciphertext block must be 16 bytes')
        cipherblock = _string_to_bytes(ciphertext)
        plaintext=[(p^l)for(p,l)in zip(self.decrypt_internal(cipherblock),self._last_cipherblock)]
