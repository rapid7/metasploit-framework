...
module Msf
  module RPC
    class RPC_Module < RPC_Base
      ...
      def rpc_exploits
        { 'modules' => framework.exploits.module_refnames }
      end
      ...
      def rpc_evasion
        { 'modules' => framework.evasion.module_refnames }
      end

      # Returns a list of persistence module names. The 'persistence/' prefix will not be included.
      #
      # @return [Hash] A list of persistence module names. It contains the following key:
      #  * 'modules' [Array<string>] Persistence names, for example: ['windows/wmi_persistence']
      # @example Here's how you would use this from the client:
      #  rpc.call('module.persistence')
      def rpc_persistence
        { 'modules' => framework.persistence.module_refnames }
      end
      ...
      def rpc_auxiliary
        { 'modules' => framework.auxiliary.module_refnames }
      end
      ...
      def rpc_post
        { 'modules' => framework.post.module_refnames }
      end
      ...
      # Returns the compatible payloads for a specific persistence module.
      #
      # @param [String] mname Persistence module name. For example: 'windows/wmi_persistence'
      # @raise [Msf::RPC::Exception] Module not found (wrong name).
      # @return [Hash] The evasion module's compatible payloads. It contains the following key:
      #  * 'payloads' [Array<String>] A list of payloads.
      # @example Here's how you would use this from the client:
      #  rpc.call('module.compatible_persistence_payloads', 'windows/wmi_persistence')
      def rpc_compatible_persistence_payloads(mname)
        m = _find_module('persistence', mname)
        res = {}
        res['payloads'] = []
      ...
      def rpc_running_stats
        {
          'waiting' => job_status_tracker.waiting_ids,
          'running' => job_status_tracker.running_ids,
          'results' => job_status_tracker.result_ids
        }
      end
      ...
      def rpc_results(uuid)
        if (r = job_status_tracker.result(uuid))
          if r[:error]
            { 'status' => 'errored', 'error' => r[:error] }
          elsif r[:result] && r[:result].length == 1
        ...
        elsif job_status_tracker.running? uuid
          { 'status' => 'running' }
        elsif job_status_tracker.waiting? uuid
          { 'status' => 'ready' }
        else
      ...
      def rpc_ack(uuid)
        { 'success' => !!job_status_tracker.ack(uuid) }
      end
      ...
      def rpc_encode(data, encoder, options)
        ...
        if (fmt = options['format']) && !supported_formats.include?(fmt)
      ...
      def _find_module(mtype, mname)
        if mname !~ %r{^(exploit|payload|nop|encoder|auxiliary|post|evasion|persistence)/}
          mname = mtype + '/' + mname
      ...
      def _run_persistence(mod, opts)
        Msf::Simple::Persistence.run_simple(mod, {
          'Payload' => opts['PAYLOAD'],
          'Target' => opts['TARGET'],
          'RunAsJob' => true,
          'Options' => opts
