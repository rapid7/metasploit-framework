...
private
	...
	def opts_to_hosts(opts)
		...
		if opts[:host] or opts[:address]
			...
			hosts << hent if hent.class == Msm::Host
		...
		if opts[:host] or opts[:address] or opts[:addresses]
			...
			hosts.each do |h|
				if opts[:port] or opts[:proto]
					...
					services << sret if sret.class == Msm::Service
				else
					services |= h.services
				end
			end
...
public
	...
	def rpc_get_note(xopts)
		...
		if(opts[:proto] && opts[:port])
			...
			services << nret if nret.class == Msm::Service
			...
			services.each do |s|
				...
				next if nret == nil
				notes << nret if nret.class == Msm::Note
	...
	def rpc_del_vuln(xopts)
		...
		if opts[:port] or opts[:proto]
			services.each do |s|
				...
				next if vret == nil
				vulns << vret if vret.class == Msm::Vuln
	...
	def rpc_del_service(xopts)
		...
		if opts[:host] or opts[:address]
			host = opts[:host] || opts[:address]
			hent = wspace.hosts.find_by_address(host)
			return { :result => 'failed' } if hent == nil or hent.class != Msm::Host
			hosts << hent
		elsif opts[:addresses]
		...
		if opts[:addresses] or opts[:address] or opts[:host]
			...
			conditions[:proto] = opts[:proto] if opts[:proto]
			sret = wspace.services.all(:conditions => conditions)
			services << sret if sret and sret.class == Msm::Service
	...
	def rpc_creds(xopts)
		...
		ret = {}
		ret[:creds] = []
		Msm::Cred.find(:all, :include => {:service => :host}, :conditions => ["hosts.workspace_id = ?",
	...
	def rpc_del_client(xopts)
		...
		hosts.each do |h|
			...
			next if cret == nil
			clients << cret if cret.class == Msm::Client
