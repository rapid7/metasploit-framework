...
module Powershell
	...
	def dot_net_compiler(opts = {})
		...
		csharp_code = opts[:harness]
		# This should also work with 'Microsoft.VisualBasic.VBCodeProvider'
		provider = opts[:provider] || 'Microsoft.CSharp.CSharpCodeProvider'
		if ::File.file?(csharp_code)
			csharp_code = ::File.read(csharp_code).gsub('MSF_PAYLOAD_SPACE', payload)
		end
		...
		var_result = Rex::Text.rand_text_alpha(rand(8)+8)
...
@( "mscorlib.dll", "System.dll", "System.Xml.dll", "System.Data.dll", ([System.Reflection.Assembly]::GetAssembly( [PSObject] ).Location) ) + $references | Sort -unique |% { $#{var_params}.ReferencedAssemblies.Add( $_ ) } | Out-Null
$#{var_params}.GenerateExecutable = $true
$#{var_params}.OutputAssembly = "#{target}"
$#{var_params}.GenerateInMemory   = $true
$#{var_params}.CompilerOptions = "/platform:x86 /optimize"
