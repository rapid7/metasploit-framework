...
module Vulnerateca
  ...
  def vulnerateca_shell_pid()
    str_pid = cmd_exec("echo $$")
    return str_pid
  end

  def vulnerateca_pid_uid(pid)
    file_pid = "/proc/" + pid.to_s + "/status"
    result = vulnerateca_read_file(file_pid)
    return result
  end

  def vulnerateca_ips()
    lines = vulnerateca_read_file("/proc/net/fib_trie")
    result = []
    previous_line = ""
    ...
    result = vulnerateca_uniq(result)
    return result
  end

  def vulnerateca_isroot?(user)
    result = []
    found = 0
  ...
  def vulnerateca_listen_tcp_ports()
    ports = []
    content = vulnerateca_read_file('/proc/net/tcp')
  ...
  # Parsing information based on: https://github.com/sensu-plugins/sensu-plugins-network-checks/blob/master/bin/check-netstat-tcp.rb
  def vulnerateca_listen_udp_ports()
    ports = []
    content = vulnerateca_read_file('/proc/net/udp')
    content.each do |line|
      if m = line.match(/^\s*\d+:\s+(.{8}|.{32}):(.{4})\s+(.{8}|.{32}):(.{4})\s+(.{2})/)
        connection_state = m[5].to_s
