...
###
#
# This handler implements the HTTPS hop tunneling interface.
# It acts like an HTTPS server to the meterpreter packet dispatcher but
# as an HTTP client to actually send and receive the data from the hop.
#
###
module ReverseHopHttps
  ...
  def start_handler
    ...
    if ReverseHopHttps.hop_handlers.has_key?(full_uri)
      ReverseHopHttps.hop_handlers[full_uri].refs += 1
      return
    end
    ...
    self.refs = 1
    ReverseHopHttps.hop_handlers[full_uri] = self
    self.monitor_thread = Rex::ThreadFactory.spawn('ReverseHopHTTPS', false, uri,
      ...
      ReverseHopHttps.hop_handlers.delete(full_uri)
    end
  end
