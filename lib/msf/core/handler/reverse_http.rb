...
module ReverseHttp
	...
	def initialize(info = {})
		...
		register_advanced_options(
			[
				...
				OptAddress.new('HttpListenerBindAddress', [ false, 'The specific IP address to bind to on the local system'])
			], Msf::Handler::ReverseHttp)
	end
	...
	def setup_handler
		...
		# Determine where to bind the server to
		bindaddrs = ipv6 ? '::' : '0.0.0.0'
		 
		if not datastore['HttpListenerBindAddress'].to_s.empty?
			bindaddrs = datastore['HttpListenerBindAddress']
		end
