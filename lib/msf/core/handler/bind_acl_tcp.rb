...
###
#
# This module implements the Bind ACL TCP handler.  This means that
...
module BindAclTcp
  ...
  def initialize(info = {})
    ...
    register_options(
      [
        ...
        OptAddress.new('AHOST', [true,  'IP address allowed to get the shell', '192.168.1.33']),
      ], Msf::Handler::BindAclTcp)
  ...
  def start_handler
    ...
    self.listener_threads << framework.threads.spawn("BindTcpHandlerListener-#{lport}", false) {
      ...
      print_status("Started ACL bind handler")
      ...
      if (client)
        ...
        conn_threads << framework.threads.spawn("BindTcpHandlerSession", false, client) { |client_copy|
          begin
            ...
            elog("Exception raised from BindAclTcp.handle_connection: #{$!}")
          end
        }
      else
