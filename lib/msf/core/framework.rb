...
module Msf
  ...
  class Framework
    ...
    def initialize(options = {})
      ...
      if options.include?('CustomDnsResolver') && Msf::FeatureManager.instance.enabled?(Msf::FeatureManager::DNS)
        ...
        dns_resolver.set_framework(self)
        Rex::Socket._install_global_resolver(dns_resolver)
      end
    ...
    #
    # Returns the module set for evasion modules
    #
    def evasion
    ...
    attr_writer :dns_resolver, :events, :modules, :datastore, :auxmgr, :jobs, :plugins, :db, :browser_profiles, :analyze, :features, :history_manager # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc: # :nodoc:
  ...
  class FrameworkEventSubscriber
    ...
    def on_session_output(session, output)
      ...
      chunks = []
      if buff.length > 1024
        chunks << buff.slice!(0, 1024) while buff.length > 0
      else
        chunks << buff
      end
