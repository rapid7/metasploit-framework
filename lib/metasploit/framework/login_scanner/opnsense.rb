    ...
    module LoginScanner
      ...
      class OPNSense < HTTP
        ...
        def send_request(opts, keep_cookies = false)
          ...
          if keep_cookies && res
            @php_sessid = res.get_cookies.split('; ').find { |cookie| cookie.start_with?('PHPSESSID') }.split('=').last
            @cookie_test = res.get_cookies.split('; ').find { |cookie| cookie.start_with?('cookie_test') }.split('=').last
          end
