...
class Console::CommandDispatcher::Stdapi::CorrMApps
  ...
  def commands
    all = {
      "app_list"      => "List installed apps in android device",
  ...
  def cmd_app_install(*args)
    ...
    app_uninstall_opts.parse(args) do |opt, _idx, val|
      ...
      when "-h"
        ...
        print_line("ex. app_install -p '/storage/sdcard/Download/corrm.apk'")
      ...
      when "-p"
        ...
        case client.apps.app_install(full_path)
        when 1
          print_good('Request Done #!!!#')
        when 2
  ...
  #
  def to_table(data)
    column_headers = [ "Package", "Name", "Running", "IsSystem" ]
