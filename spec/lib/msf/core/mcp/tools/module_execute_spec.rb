...
RSpec.describe Msf::MCP::Tools::ModuleExecute do
  ...
  describe 'Output Schema' do
    it 'declares job_id as nullable integer' do
      job_id_schema = described_class.output_schema.schema[:properties][:data][:properties][:job_id]
      expect(job_id_schema[:type]).to match_array(%w[integer null])
    end

    it 'declares uuid as nullable string' do
      uuid_schema = described_class.output_schema.schema[:properties][:data][:properties][:uuid]
      expect(uuid_schema[:type]).to match_array(%w[string null])
    end
  end
  ...
  describe '.call' do
    ...
    it 'propagates null job_id and uuid when the RPC swallowed an early driver failure' do
      allow(msf_client).to receive(:module_execute).and_return({ 'job_id' => nil, 'uuid' => nil })

      result = described_class.call(type: 'exploit', name: 'multi/handler', options: {}, server_context: server_context)

      expect(result.error?).to be false
      expect(result.structured_content[:data][:job_id]).to be_nil
      expect(result.structured_content[:data][:uuid]).to be_nil
    end
