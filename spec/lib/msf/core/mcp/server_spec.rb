...
RSpec.describe Msf::MCP::Server do
  ...
  describe '#start' do
    ...
    context 'with http transport' do
      ...
      before do
        # Stub #require to prevent actually loading rack and puma
        allow(server).to receive(:require).with('rack').and_return(true)
        allow(server).to receive(:require).with('puma').and_return(true)
        allow(server).to receive(:require).with('puma/configuration').and_return(true)
        allow(server).to receive(:require).with('puma/launcher').and_return(true)
        allow(server).to receive(:require).with('puma/log_writer').and_return(true)
