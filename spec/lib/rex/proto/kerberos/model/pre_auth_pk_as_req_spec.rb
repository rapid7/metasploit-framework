...
#  describe "#parse" do
#    context "when parsing PA-PK-AS-REQ" do
#      it "returns the Rex::Proto::Kerberos::Model::PreAuthPkAsReq decoded" do
#        result = Rex::Proto::Kerberos::Model::PreAuthPkAsReq.parse(sample)
#        expect(result[:signed_auth_pack].value[:signed_data].value[:version].value).to eq(3)
#        expect(result[:signed_auth_pack].value[:signed_data].value[:digest_algorithms].value.length).to eq(1)
#      end
#    end
#  end

  describe "#encode" do
    context "when parsing then re-encoding PA-PK-AS-REQ" do
      it "returns the same value it started with" do
        result = Rex::Proto::Kerberos::Model::PreAuthPkAsReq.parse(sample)
        print("SAP is: #{result}\n")
