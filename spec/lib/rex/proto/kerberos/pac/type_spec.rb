  ...
  describe '#encode' do
    context 'when RSA-MD5 checksums' do
      it 'encodes the PAC-TYPE correctly' do
        logon_info = Rex::Proto::Kerberos::Pac::Krb5ValidationInfo.new(
        ...
        x = described_class.new(
          # pac_info_buffers: pac_info_buffers,
          pac_elements: pac_elements
        )
        x.calculate_checksums
        # x.c_buffers = pac_info_buffers.length
        # x.pac_info_buffers = pac_info_buffers
        # pp x
        # x.pac_elements = pac_elements

        # pac_type.checksum = rsa_md5

        # expect(x.to_binary_s).to eq(sample)
        expect(x).to eq(Rex::Proto::Kerberos::Pac::Krb5Pac.read(sample))
      end
    end
  end
