RSpec.describe Rex::Proto::MsDtyp::MsDtypAce do
  ...
  describe '.from_sddl_text' do
    it 'raises an exception on invalid ACEs' do
      expect { described_class.send(:from_sddl_text, '', domain_sid: domain_sid) }.to raise_error(RuntimeError)
      expect { described_class.send(:from_sddl_text, ';;;;;', domain_sid: domain_sid) }.to raise_error(RuntimeError)
      expect { described_class.send(:from_sddl_text, ';;;;;;;', domain_sid: domain_sid) }.to raise_error(RuntimeError)
    end
    ...
    context 'when parsing the ACE type' do
      it 'raises an exception on an invalid type' do
        expect { described_class.send(:from_sddl_text, 'X;;;;;', domain_sid: domain_sid) }.to raise_error(RuntimeError)
      end

      it 'sets the type correctly for A' do
        expect(described_class.send(:from_sddl_text, 'A;;;;;', domain_sid: domain_sid).header.ace_type).to eq Rex::Proto::MsDtyp::MsDtypAceType::ACCESS_ALLOWED_ACE_TYPE
      end
      ...
      it 'sets the type correctly for D' do
        expect(described_class.send(:from_sddl_text, 'D;;;;;', domain_sid: domain_sid).header.ace_type).to eq Rex::Proto::MsDtyp::MsDtypAceType::ACCESS_DENIED_ACE_TYPE
      end

      it 'sets the type correctly for OA' do
        expect(described_class.send(:from_sddl_text, 'OA;;;;;', domain_sid: domain_sid).header.ace_type).to eq Rex::Proto::MsDtyp::MsDtypAceType::ACCESS_ALLOWED_OBJECT_ACE_TYPE
      end

      it 'sets the type correctly for OD' do
        expect(described_class.send(:from_sddl_text, 'OD;;;;;', domain_sid: domain_sid).header.ace_type).to eq Rex::Proto::MsDtyp::MsDtypAceType::ACCESS_DENIED_OBJECT_ACE_TYPE
      end

      it 'sets the type correctly for AU' do
        expect(described_class.send(:from_sddl_text, 'AU;;;;;', domain_sid: domain_sid).header.ace_type).to eq Rex::Proto::MsDtyp::MsDtypAceType::SYSTEM_AUDIT_ACE_TYPE
      end

      it 'sets the type correctly for OU' do
        expect(described_class.send(:from_sddl_text, 'OU;;;;;', domain_sid: domain_sid).header.ace_type).to eq Rex::Proto::MsDtyp::MsDtypAceType::SYSTEM_AUDIT_OBJECT_ACE_TYPE
      end
    end

    context 'when parsing the ACE flags' do
      it 'raises an exception on invalid flags' do
        expect { described_class.send(:from_sddl_text, 'A;XX;;;;', domain_sid: domain_sid) }.to raise_error(RuntimeError)
      end

      it 'sets no flags by default' do
        expect(described_class.send(:from_sddl_text, 'A;;;;;', domain_sid: domain_sid).header.ace_flags.snapshot.values.sum).to eq 0
      end

      it 'sets the flag correctly for CI' do
        expect(described_class.send(:from_sddl_text, 'A;CI;;;;', domain_sid: domain_sid).header.ace_flags.container_inherit_ace).to eq 1
      end
      ...
      it 'sets the flag correctly for OI' do
        expect(described_class.send(:from_sddl_text, 'A;OI;;;;', domain_sid: domain_sid).header.ace_flags.object_inherit_ace).to eq 1
      end

      it 'sets the flag correctly for NP' do
        expect(described_class.send(:from_sddl_text, 'A;NP;;;;', domain_sid: domain_sid).header.ace_flags.no_propagate_inherit_ace).to eq 1
      end

      it 'sets the flag correctly for IO' do
        expect(described_class.send(:from_sddl_text, 'A;IO;;;;', domain_sid: domain_sid).header.ace_flags.inherit_only_ace).to eq 1
      end

      it 'sets the flag correctly for ID' do
        expect(described_class.send(:from_sddl_text, 'A;ID;;;;', domain_sid: domain_sid).header.ace_flags.inherited_ace).to eq 1
      end

      it 'sets the flag correctly for SA' do
        expect(described_class.send(:from_sddl_text, 'A;SA;;;;', domain_sid: domain_sid).header.ace_flags.successful_access_ace_flag).to eq 1
      end

      it 'sets the flag correctly for FA' do
        expect(described_class.send(:from_sddl_text, 'A;FA;;;;', domain_sid: domain_sid).header.ace_flags.failed_access_ace_flag).to eq 1
      end

      it 'sets the flag correctly for CR' do
        expect(described_class.send(:from_sddl_text, 'A;CR;;;;', domain_sid: domain_sid).header.ace_flags.critical_ace_flag).to eq 1
      end
    end
    ...
    context 'when parsing the ACE rights' do
      ...
      it 'sets no rights by default' do
        expect(described_class.send(:from_sddl_text, 'A;;;;;', domain_sid: domain_sid).body.access_mask).to eq Rex::Proto::MsDtyp::MsDtypAccessMask::NONE
      end

      %w[ GA GR GW GX ].each do |right|
        it "sets the rights correctly for #{right}" do
          expect(described_class.send(:from_sddl_text, "A;;#{right};;;", domain_sid: domain_sid).body.access_mask.send(right.downcase)).to eq 1
        end
      end

      it 'sets the rights correctly for FA' do
        expect(described_class.send(:from_sddl_text, 'A;;FA;;;', domain_sid: domain_sid).body.access_mask.protocol).to eq 0x1ff
        expect(described_class.send(:from_sddl_text, 'A;;FA;;;', domain_sid: domain_sid).body.access_mask.de).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;FA;;;', domain_sid: domain_sid).body.access_mask.rc).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;FA;;;', domain_sid: domain_sid).body.access_mask.wd).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;FA;;;', domain_sid: domain_sid).body.access_mask.wo).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;FA;;;', domain_sid: domain_sid).body.access_mask.sy).to eq 1
      end

      it 'sets the rights correctly for KA' do
        expect(described_class.send(:from_sddl_text, 'A;;KA;;;', domain_sid: domain_sid).body.access_mask.protocol).to eq 0x3f
        expect(described_class.send(:from_sddl_text, 'A;;KA;;;', domain_sid: domain_sid).body.access_mask.de).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;KA;;;', domain_sid: domain_sid).body.access_mask.rc).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;KA;;;', domain_sid: domain_sid).body.access_mask.wd).to eq 1
        expect(described_class.send(:from_sddl_text, 'A;;KA;;;', domain_sid: domain_sid).body.access_mask.wo).to eq 1
      end
    end
    ...
    context 'when parsing the ACE object GUID' do
      ...
      it 'raises an exception when the ACE type is incompatible' do
        expect { described_class.send(:from_sddl_text, "A;;;#{dummy_guid};;", domain_sid: domain_sid) }.to raise_error(RuntimeError)
      end

      it 'sets no object GUID by default' do
        expect(described_class.send(:from_sddl_text, "OA;;;;;", domain_sid: domain_sid).body.flags.ace_object_type_present).to eq 0
        expect(described_class.send(:from_sddl_text, "OA;;;;;", domain_sid: domain_sid).body.object_type).to eq '00000000-0000-0000-0000-000000000000'
      end

      it 'sets the object type' do
        expect(described_class.send(:from_sddl_text, "OA;;;#{dummy_guid};;", domain_sid: domain_sid).body.flags.ace_object_type_present).to eq 1
        expect(described_class.send(:from_sddl_text, "OA;;;#{dummy_guid};;", domain_sid: domain_sid).body.object_type).to eq dummy_guid
      end
    end
    ...
    context 'when parsing the ACE inherited object GUID' do
      ...
      it 'sets no inherited object GUID by default' do
        expect(described_class.send(:from_sddl_text, "OA;;;;;", domain_sid: domain_sid).body.flags.ace_inherited_object_type_present).to eq 0
        expect(described_class.send(:from_sddl_text, "OA;;;;;", domain_sid: domain_sid).body.inherited_object_type).to eq '00000000-0000-0000-0000-000000000000'
      end

      it 'sets the inherited object type' do
        expect(described_class.send(:from_sddl_text, "OA;;;;#{dummy_guid};", domain_sid: domain_sid).body.flags.ace_inherited_object_type_present).to eq 1
        expect(described_class.send(:from_sddl_text, "OA;;;;#{dummy_guid};", domain_sid: domain_sid).body.inherited_object_type).to eq dummy_guid
      end
    end
    ...
    context 'when parsing the ACE SID' do
      ...
      it 'calls .from_sddl_text' do
        ...
        expect(described_class.send(:from_sddl_text, "A;;;;;#{dummy_sid}", domain_sid: domain_sid).body.sid).to eq dummy_sid
      end
    end
  end
