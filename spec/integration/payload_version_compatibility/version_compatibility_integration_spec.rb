...
RSpec.describe 'Payload version compatibility integration' do
  ...
  let(:mock_module_info_with_minimum_versions) { { 'Name' => 'Mock Payload', 'MinimumVersions' => { 'Windows' => Msf::WindowsVersion::XP_SP2 } } }

  # Mock payload
  let(:payload_with_minimum_versions) do
    instance = instance_double(Msf::Payload)
    allow(instance).to receive(:instance_variable_get).with(:@module_info).and_return(mock_module_info_with_minimum_versions)
    instance
  end

  let(:payload_without_minimum_versions) do
    instance = instance_double(Msf::Payload)
    allow(instance).to receive(:instance_variable_get).with(:@module_info).and_return(mock_module_info_without_minimum_versions)
    instance
  end
  ...
  describe '#version_compatibility_warnings' do
    context 'when target is Windows 2000 (below Meterpreter minimum)' do
      ...
      it 'warns for payload with MinimumVersions' do
      ...
      it 'does not warn for payload without MinimumVersions' do
    ...
    context 'when target is Windows XP SP2 (meets Meterpreter minimum)' do
      ...
      it 'does not warn for payload with MinimumVersions' do
        warnings = exploit.version_compatibility_warnings(payload_with_minimum_versions)
        expect(warnings).to be_empty
      end
    end
