...
describe Metasploit::Framework::Module::Path::Loader::Base do
	include_context 'Metasploit::Framework::Module::Path::Loader::Base'
	...
	context 'instance methods' do
					...
					context 'with other Exception' do
						...
						context 'without version compatibility' do
							let(:version_compatibility_error) do
								Metasploit::Framework::Module::Ancestor::Error::VersionIncompatibility.new(
				...
				context 'without module_eval errors' do
					...
					context 'with version compatibility' do
						...
						context 'without metasploit_class' do
							let(:error) do
								Metasploit::Framework::Module::Ancestor::Error::MetasploitModuleIncompatibility.new(
										:module_path => module_path,
										:module_reference_name => module_reference_name
								)
							end
							...
							it 'should record load error' do
								subject.should_receive(
										...
										module_path,
										kind_of(Metasploit::Module::Ancestor::Error::MetasploitModuleIncompatibility)
								)
								subject.load_module(parent_path, type, module_reference_name).should be_false
							end
		...
		context '#module_path?' do
			it 'should return false if path is hidden' do
				...
				subject.send(:ancestor_path?, hidden_path).should be_false
			end
			...
			it 'should return false if the file extension is not MODULE_EXTENSION' do
				...
				subject.send(:ancestor_path?, path).should be_false
			end
			...
			it 'should return true otherwise' do
				subject.send(:ancestor_path?, module_path).should be_true
			end
		end
		...
		context '#module_reference_name_from_path' do
			it 'should strip MODULE_EXTENSION from the end of the path' do
				...
				subject.send(:module_ancestor_reference_name_from_path, path).should == path_without_extension
			end
		end
