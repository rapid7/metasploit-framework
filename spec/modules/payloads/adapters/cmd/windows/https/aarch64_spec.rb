...
RSpec.describe 'cmd/windows/https/aarch64' do
  include_context 'Msf::Simple::Framework#modules loading'

  # Adapter payloads cannot be instantiated standalone; they must be combined
  # with a compatible single payload. Windows AArch64 has no Meterpreter yet,
  # so we use windows/aarch64/shell_reverse_tcp.
  let(:subject) do
    load_and_create_module(
      module_type: 'payload',
      reference_name: 'cmd/windows/https/aarch64/shell_reverse_tcp',
  ...
  describe 'module metadata' do
    ...
    it 'adapts AArch64 payloads' do
      expect(subject.send(:module_info)['AdaptedArch']).to eq(ARCH_AARCH64)
    end
    ...
    it 'does not adapt x64 or x86 payloads' do
