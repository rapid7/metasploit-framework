...
class MetasploitModule < Msf::Auxiliary
  ...
  def report_creds(
    user, hash, type: :ntlm_hash, jtr_format: '', realm_key: nil, realm_value: nil, is_login_cred: true
  )
    service_data = {
      address: rhost,
      port: rport,
    ...
    if upper_name.start_with?('_SC_')
      ...
      if machine_kerberos_keys.empty?
        ...
        machine_kerberos_keys.each do |key|
          unless report_creds(print_name, key, is_login_cred: false, **credential_opts)
