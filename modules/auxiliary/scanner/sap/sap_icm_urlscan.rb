...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(_ip)
    ...
    if res
      ...
      if res.headers['Server']
        ...
        print_status("Server responded with the following Server Header: #{@info[0]}")
      else
        print_status("Server responded with a blank or missing Server Header")
      end

      if res.body && /class="note">(.*)code:(.*)</i.match(res.body)
        print_error("SAP ICM error message: #{::Regexp.last_match(2)}")
      end
      ...
      print_status("Beginning URL check")
      @valid_urls = ''
  ...
  def check_url(url)
    ...
    if res
      if res.headers['Server'] && !@info.include?(res.headers['Server'])
  ...
  def bypass_auth(url)
    ...
    if res&.code == 200
      ...
      vprint_status("Could not get authentication bypass via HTTP verb tampering")
    end
  end
  ...
  def check_urlprefixes
    ...
    if res&.code == 200
      ...
      print_error("Could not retrieve urlprefixes")
    end
