...
class Metasploit4 < Msf::Auxiliary
	...
	def exec()
		...
		print_status("#{datastore['RHOSTS']}:#{datastore['RPORT']} - sending SOAP TH_SAPREL request")
		begin
			...
			if res.code == 500
				...
				print_error("#{rhost}:#{rport} - error message: " + res.code.to_s + " " + res.message)
			end
		...
		if success == false
			...
			print_error("#{datastore['RHOSTS']}:#{datastore['RPORT']} - #{err.gsub('&#39;','\'')}")
		end
	end
end