...
class MetasploitModule < Msf::Auxiliary
  ...
  def extractusers(rhost)
    ...
    begin
      ...
      print_error("Unable to attempt authentication on #{rhost}:#{rport}")
      return
    end

    if success
      print_good("Users Extracted: #{users.length} entries extracted from #{rhost}:#{rport}")
      report_note(
        host: rhost,
        proto: 'tcp',
        port: rport,
      ...
      users.each do |output|
        print_good("Extracted User: #{output[0]}")
      end
    elsif fault
