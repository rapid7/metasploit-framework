...
class Metasploit4 < Msf::Auxiliary
	...
	def initialize
		...
		register_options(
			[
				Opt::RPORT(3299),
				OptString.new('RHOSTS', [true, 'SAP Router Address', nil]),
				OptString.new('RPORT', [true, 'SAP Router Port Number', '3299']),
	...
	def run_host(ip)
		...
		destination = ''
		service = ''
		ni_packet = type << [0].pack('c*') << [version].pack('c*') << [cmd].pack('c*') << [0,0].pack('c*')
		ni_packet = [ni_packet.length].pack('N') << ni_packet
		saptbl = Msf::Ui::Console::Table.new(
			Msf::Ui::Console::Table::Style::Default,
				'Header' => "[SAP] SAProuter Connection Table for #{target}",
				'Prefix' => "\n",
				'Postfix' => "\n",
				'Indent' => 1,
				'Columns' =>
		...
		begin
			...
			print_status("#{datastore['RHOSTS']}:#{datastore['RPORT']} - connection refused")
			connected == 'false'
		rescue ::Rex::ConnectionError, ::IOError, ::Timeout::Error
			print_status("#{datastore['RHOSTS']}:#{datastore['RPORT']} - connection timeout")
			connected == 'false'
		rescue ::Exception => e
			print_error("#{datastore['RHOSTS']}:#{datastore['RPORT']} - exception #{e.class} #{e} #{e.backtrace}")
			connected == 'false'
		end
		if connected != true
