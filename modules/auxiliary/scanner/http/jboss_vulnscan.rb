...
class MetasploitModule < Msf::Auxiliary
  ...
  def check_app(app)
    ...
    case
    when res.code == 200
      print_good("#{app} does not require authentication (200)")
    when res.code == 403
    ...
    when res.code == 500 && app == "/invoker/readonly"
      print_good("#{app} responded (#{res.code})")
    else
  ...
  def jboss_as_default_creds
    print_status("Checking for JBoss AS default creds")
    ...
    if res && res.code == 302 && res.headers.to_s !~ /error.seam/m && res.headers.to_s =~ /summary.seam/m
      print_good("Authenticated using #{username}:#{password} at /admin-console/")
  ...
  def bypass_auth(app)
    print_status("Check for verb tampering (#{datastore['VERB']})")
    ...
    if res && res.code == 200
      print_good("Got authentication bypass via HTTP verb tampering at #{app}")
    else
  ...
  def basic_auth_default_creds(app)
    ...
    if res && res.code == 200
      print_good("Authenticated using admin:admin at #{app}")
