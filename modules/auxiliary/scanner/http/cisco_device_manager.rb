...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    if res and res.code == 401
      print_error("Failed to authenticate to this device")
      return
    end

    if res and res.code != 200
      print_error("Unexpected response code from this device #{res.code}")
      return
    end

    if res and res.body and res.body =~ /Cisco (Internetwork Operating System|IOS) Software/
      print_good("Successfully authenticated to this device")
