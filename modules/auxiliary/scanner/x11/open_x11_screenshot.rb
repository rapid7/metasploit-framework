...
class MetasploitModule < Msf::Auxiliary
  ...
  def take_screenshot(connection)
    ...
    @all_windows.reverse.each do |window|
      ...
      print_good("Found background image: #{window['window_id']} (#{window['window_int']})")
      sock.put(GETIMAGEREQUEST.new(height: window['height'],
                                   width: window['width'],
                                   drawable: window['window_int']).to_binary_s)
      GETIMAGERESPONSE.read(sock.get_once(-1, 1))
