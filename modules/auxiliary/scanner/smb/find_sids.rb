...
class Metasploit3 < Msf::Auxiliary
	...
	def initialize
		...
		register_options(
			[
				OptInt.new('MaxRID', [ false, "Maximum RID to check", 2000 ]),
				OptPath.new('USER_FILE', [ false, "Line-separated file list of users" ]),
				OptString.new('USERNAME', [ false, "Single user to search for" ])
			],
			self.class
		)
	...
	def run_host(ip)
		...
		begin
			...
			500.upto(datastore['MaxRID'].to_i) do |rid|
				...
				if userlist.include? uname
					print_good("#{ip} - Found #{uname} - RID=#{rid}")
				end

				case utype
				when 1
					vprint_status("#{ip} USER=#{uname} RID=#{rid}")
					domain[:users][rid] = uname
				when 2
					domain[:groups][rid] = uname
					vprint_status("#{ip} GROUP=#{uname} RID=#{rid}")
				else
					vprint_status("#{ip} TYPE=#{utype} NAME=#{uname} rid=#{rid}")
				end
			end
			...
			vprint_status("#{ip} #{domain[:name].upcase} [#{domain[:users].keys.map{|k| domain[:users][k]}.join(", ")} ]")
