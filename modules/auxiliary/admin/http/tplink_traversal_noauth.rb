...
class Metasploit3 < Msf::Auxiliary
	...
	def find_files(files)
		...
		if (res and res.code == 200)
			out = false
			print_good("#{rhost}:#{rport} - Request may have succeeded file->#{files}! Response: \r\n")
			...
			vprint_error("#{rhost}:#{rport} - Attempt returned HTTP error #{res.code} on file->#{files}")
		end
	end
