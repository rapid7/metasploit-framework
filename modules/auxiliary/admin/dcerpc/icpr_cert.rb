...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize(info = {})
    ...
    register_advanced_options(
      [
        OptEnum.new('UPDATE_LDAP_OBJECT', [ false, 'Either userPrincipalName or dNSHostName, Updates the necessary object of a specific user before requesting the cert. Used to exploit ESC9 and ESC10. ', 'userPrincipalName', %w(userPrincipalName dNSHostName) ]),
        ...
        OptString.new('TARGET_USERNAME', [false, 'The username of the target LDAP object.'], conditions: %w[!UPDATE_LDAP_OBJECT.nil?]),
        OptString.new('NEW_VALUE', [false, 'The new value for the specified attribute.'], conditions: %w[!UPDATE_LDAP_OBJECT.nil?])
      ]
    )
  end
