...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    external_map = "#{external_client || 'any'}:#{external_port}/#{protocol}"
    internal_map = "#{internal_client || 'any'}:#{internal_port}/#{protocol}"
    map = "#{external_map} -> #{internal_map}"

    if res
      if res.code == 200
        print_good("#{map} #{action.name} succeeded")
      else
        print_error("#{map} #{action.name} failed with response code #{res.code}")
        vprint_status(res.body.to_s)
      end
    else
      print_error("no response for #{map} #{action.name}")
    end
  end
end
