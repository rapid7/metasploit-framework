...
class MetasploitModule < Msf::Post
  ...
  def initialize(info = {})
    super(update_info(info,
      'Name' => 'Orchestrator secret variables decryptor',
      'Description' => %q{
        It is possible to store variables with encrypted content within Orchestrator. The encryption key is stored in
        the SYS table. If you have access to the SYS table it is possible to decrypt the secret variables.
    ...
    register_options(
      [
        ...
        OptString.new('DATABASE', [true, 'Name of Orchestrator database','Orchestrator']),
  ...
  def run
    ...
    base_script = File.read(File.join(Msf::Config.data_directory, "post", "powershell", "Decrypt-OrchestratorVariables.ps1"))
