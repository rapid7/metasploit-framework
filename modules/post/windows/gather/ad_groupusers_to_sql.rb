        ...
        group_gather << framework.threads.spawn("Module(#{refname})", false, remaining_groups.shift) do |individual_group|
          begin
            ...
            sql_param_group = { g_rid: group_rid,
                                ...
                                g_sAMAccountName: individual_group[3][:value].encode('UTF-8'),
                                g_whenChanged: individual_group[4][:value].encode('UTF-8'),
                                g_whenCreated: individual_group[5][:value].encode('UTF-8'),
            ...
            users_in_group[:results].each do |group_user|
              ...
              sql_param_user = { u_rid: user_rid,
                                 u_distinguishedName: group_user[0][:value].encode('UTF-8'),
                                 u_sAMAccountType: group_user[2][:value].to_i,
                                 u_sAMAccountName: group_user[3][:value].encode('UTF-8'),
                                 u_displayName: group_user[4][:value].encode('UTF-8'),
                                 u_description: group_user[5][:value].encode('UTF-8'),
                                 u_logonCount: group_user[6][:value].to_i,
                                 u_userAccountControl: uac_int,
                                 u_userPrincipalName: group_user[8][:value].encode('UTF-8'),
                                 u_whenChanged: group_user[9][:value].encode('UTF-8'),
    ...
    begin
      ...
      computers[:results].each do |comp|
        ...
        sql_param_computer = { c_rid: computer_rid,
                               c_distinguishedName: comp[0][:value].encode('UTF-8'),
                               c_cn: comp[2][:value].encode('UTF-8'),
                               c_dNSHostName: comp[3][:value].encode('UTF-8'),
                               c_sAMAccountType: sat_int,
                               c_sAMAccountName: comp[5][:value].encode('UTF-8'),
                               c_displayName: comp[6][:value].encode('UTF-8'),
                               c_logonCount: comp[7][:value].to_i,
                               c_userAccountControl: uac_int,
                               c_whenChanged: comp[9][:value].encode('UTF-8'),
                               c_whenCreated: comp[10][:value].encode('UTF-8'),
                               c_primaryGroupID: comp[11][:value].to_i,
                               c_badPwdCount: comp[12][:value].to_i,
                               c_operatingSystem: comp[13][:value].encode('UTF-8'),
